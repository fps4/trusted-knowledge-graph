...
> Everything below runs, `make gate` passes, and every number comes from a report
> the run wrote. Extraction and the vector path's answers and verdicts are Claude
> (`claude-opus-5-5`, low effort), committed as fixtures — the demo needs no key and
> no network.
...
Mara, a partner, asks whether the firm has
...
"Active client" means four different
...
reason. If the index is not reachable, a question that needs passages is refused
and says so. ([ADR 0017](docs/decisions/0017-the-router-decides-and-says-why.md))
...
another matter. So the corpus has what a firm's does: **knowledge notes**, filed
...
Every request writes one line to `audit/decisions.jsonl`, once, at
...
**An open decision: aggregates over walls.** Refusing an aggregate over a set you
cannot fully see is right for "who has the most AFM experience". For a firm-wide
count it means nobody gets an answer: every lawyer in the lab is walled from *something* the Finance and Risk
...
- **SHACL is the load contract.** A load that violates the shapes does not land —
  ...
  assertions and identifiers, never document content. The shapes also refuse a fact without a source, confidence and review state, and a derived graph
  without lineage.
- **The model does not write SPARQL.** Questions are templates bound to competency
  questions, and every template declares every graph it touches.
...
only other service. No login and no switcher: the window is the person. A static
start page on :3100 introduces the lab, its architecture and decisions, and links to
each screen. `make up` starts them all; `make web-links HOST=<host>` prints the tunnel
and the URLs:
